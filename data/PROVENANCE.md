...
downloads can rebuild the panels exactly.
...
Place the raw ERCOT downloads in a local `data_raw/` directory and run the panel-derivation
routine (the transformation steps above are small and self-contained; the analytical
columns and window definitions are all in `ree.config`). The published derived panels then
reproduce every table and figure via `python scripts/run_all.py`.
